...
router.post("/questions/:courseCode", async ({ response, state }, next) => {
  const lastQuestionTimestamp = await rateLimitClient.GET(
    `question-${state.user}`,
  );
  ...
  if (diff < 60 * 1000) {
    ...
    return response.body = {
      error: `Only one question per minute. Please try again in ${
  ...
  await serviceClient.XADD("ai_gen_answers", "*", {
